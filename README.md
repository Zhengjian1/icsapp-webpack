...
```
 ...
 —| —| —| setupProxy    => http-proxy-middleware代理置
 —| —| webpack.common   => webpack本地和生产公共配置(如alias)
 —| —| webpack.dev      => webpack本地配置
 —| —| webpack.dll      => webpack dll打包,提升二次启效率
 —| —| webpack.prod     => webpack生产配置
 —| dist                => 本地开发的打包文件目录
 —| pubilc              => 资源文件
 —| —| asset            => template模板需要的文件(注打包后和template在同意目录下，引入js用./就可以)
 —| —| —| favicon       => 图标
 —| —| —| lib-flexible  => 淘宝的lib-flexible,做了一兼容处理
